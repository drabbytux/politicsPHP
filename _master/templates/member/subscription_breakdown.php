...
<br />
		...
		<fieldset>
				<legend>Subscription Options</legend>
					...
					<?
						...
						foreach($this->Get_Data_Holder('subscription_price_array') as $price ){
							if( !$turn ) {
								print "<strong>". $price['subscription_group_description'];
								
								//	print '<select style="font-size: 14pt;width: 100%;" name="member_subscription_prince" multiple size="'.count($this->Get_Data_Holder('subscription_price_array')).'">';
							}
							
							if( $current_paper_id!=$price['subscription_paper_id']){
								$current_paper_id=$price['subscription_paper_id'];
								print " to ".$price['subscription_paper_name'].'</strong>';
							}
							
							if( $print_or_elec!=$price['subscription_prices_paper_type_id']){
								$print_or_elec=$price['subscription_prices_paper_type_id'];
								print '<div style="border-bottom: 1px #444 solid;">'. $price['subscription_paper_type_name'].'</div>';
							}
							// 
							?>
							<div style="padding: 6px 0px;" ><input  id="price<?=$price['subscription_prices_id']; ?>" type="radio" name="member_price" value="<?=$price['subscription_prices_id']; ?>" <?=is_selected('checked', $price['subscription_prices_id'], $this->Get_Data('member_price')); ?>><label style="font-size: 12pt;"  for="price<?=$price['subscription_prices_id']; ?>">$<?=$price['subscription_prices_price'] ?> &nbsp; &nbsp;<?=$price['subscription_prices_description'] ?></label></div>								
							<?
							$selected = false;
							$turn = true;
						}
	...
	<br /><br />
		...
		<fieldset>	
				<table>
					<tr><td class="tdbuttons"><!--  <div class="button_area"><input class="button_warning" type="submit" value="cancel" name="member_subscription_profile_cancel" /></div>  --><div class="button_area"><input class="button_good" type="submit" value="Proceed to Payment Options" name="member_subscription_profile_submit" /></div></td></tr>
				</table>
		</fieldset>
