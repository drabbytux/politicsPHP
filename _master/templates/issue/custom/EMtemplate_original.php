		...
		<tr>
			<td style="width: 300px"><? // [ HOME PAGE THROW ] ?>
				<? // Find a way to remove the anchor tag if the throw source is the issue and NOT the story
					if( is_array($this->Get_Data('throw') ) || $this->get_Data('normal_throw') ) { ?>
						<? if( $this->get_data('normal_throw')  ) {?>
							<a href="/page/view/<?=$this->getIssueStoryElement(FRONT_PAGE,0,'story_url_id');?>"><img title="<?=htmlentities( $this->Get_Data('throw', 'photo_cutline') );?>" style="border: 1px #999 solid;" src="<?=$this->get_data('normal_throw') ?>" /></a>
							<? } else { ?>
	...
	<table style="width: 100%;">
		<tr>
			...
			<td><? // Master right column ?>
				...
				<?=$this->outputHeader('CULTURE', array(LISTINGS,8) ); //8 is culture ?>
				<?=$this->outputSection(11, false);?>
				<?=$this->outputSection(8, false);?>
