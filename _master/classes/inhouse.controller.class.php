...
class InHouseController extends Controller {
	...
	public function policy_briefing_side(){
		...
		$sql = "SELECT * from policy_briefing where policy_briefing_publish_date >= '".mktime()."' order by policy_briefing_publish_date asc limit 2";
		$pb_coming = $this->ExecuteAssoc( $sql );
		
		$str_out .= '<table style="width: 280px;">';
		$str_out .= (is_array($pb_latest))? '<tr><td style="background: transparent; width: 100px;"><small>'. $this->_formatDate( $pb_latest['policy_briefing_publish_date'] ) . '</small></td><td style="background: transparent; width: 200px; padding-bottom: 5px;"><a href="/reports/2008/110608_pb.pdf">'. $pb_latest['policy_briefing_title'] .'</a></td></tr>' : NULL;
		$str_out .= (is_array($pb_coming))? '<tr><td style="border-top: 1px #ccc solid; background: transparent; width: 100px;"><small>'. $this->_formatDate( $pb_coming['policy_briefing_publish_date'] ) . '</small></td><td style="border-top: 1px #ccc solid;background: transparent; width: 200px;">'. $pb_coming['policy_briefing_title'].'<br /><small>Booking date: '. $this->_formatDate( $pb_coming['policy_briefing_booking_date'] ) . '</small></td></tr>' : NULL;
		$str_out .= '</table>';
